...
def get_posts(db: Session = Depends(get_db),user_id:int = Depends(oauth2.get_current_user),limit:int = 10,skip:int = 0,
    ...
    posts = db.query(models.Post,func.count(models.Vote.post_id).label("voting")).join(models.Vote,models.Post.id == models.Vote.post_id,
...
def create_posts(post: schemas.PostCreate ,db :Session = Depends(get_db),current_user:int = Depends(oauth2.get_current_user)):
    ...
    # give me example using fstrings
    # cursor.execute("""INSERT INTO posts (title, content, published) VALUES (%s, %s, %s) RETURNING *""",
    # (post.title, post.content, post.published))
    # new_post = cursor.fetchone()
    # conn.commit()
    """ Below line is ineffecient method """
...
def get_post(id: int ,response: Response ,db :Session = Depends(get_db),current_user:int = Depends(oauth2.get_current_user)):
    ...
    post = db.query(models.Post, func.count(models.Vote.post_id).label("voting")).join(models.Vote,
